                ...
                <ul class="navbar-nav mr-auto">
                    ...
                    <li class="nav-item dropdown active">
                        ...
                        <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                            ...
                            <a class="dropdown-item active" href="">Danh sách khách hàng</a>
        ...
        <table class="table">
            ...
            <tbody>
                <c:forEach items="${list}" var="element">
					<tr>
						<td scope="row">${element.getCustomerID()}</td>
						<td scope="row">${element.getCustomerName()}</td>
						<td scope="row">${element.getAddress()}</td>
						<td scope="row">${element.getPhoneNumber()}</td>
						<td scope="row">${element.getEmail()}</td>
						<td scope="row">
						<a href="editCustomer?id=<c:out value='${element.getCustomerID()}' />"><img src='<c:url value="/resources/images/edit.png" />'></a>
                        <a href="deleteCustomer?id=<c:out value='${element.getCustomerID()}' /> " style="margin-left:20px;"><img src='<c:url value="/resources/images/quit.png" />'></a>
						</td>
					</tr>
				</c:forEach>
            </tbody>
            ...
            <ul class="pagination justify-content-end">
                <c:if test="${currentPage != 1}">
					<li class="page-item"><a class="page-link"
						href="danhsachkhachang?currentPage=${currentPage-1}">Previous</a></li>
				</c:if>
				...
				<c:forEach begin="1" end="${noOfPages}" var="i">
					<c:choose>
						...
						<c:otherwise>
							<li class="page-item"><a class="page-link"
								href="danhsachkhachhang?currentPage=${i}">${i}</a></li>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				...
				<c:if test="${currentPage lt noOfPages}">
					<li class="page-item"><a class="page-link"
						href="danhsachkhachhang?currentPage=${currentPage+1}">Next</a></li>
				</c:if>
            </ul>
        </nav>
    </div>
