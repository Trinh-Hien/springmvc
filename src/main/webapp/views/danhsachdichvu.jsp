...
<body>
				...
				<ul class="navbar-nav mr-auto">
					...
					<li class="nav-item dropdown  active"><a
						...
						<div class="dropdown-menu" aria-labelledby="navbarDropdown">
							...
							<a class="dropdown-item  active" href="">Danh sách dịch vụ</a>
	...
	<div class="container" style="margin-top: 100px; margi-bottom: 200px;">
		<table class="table">
			...
			<tbody>
				<c:forEach items="${list}" var="element">
					<tr>
						<td scope="row">${element.getServiceID()}</td>
						<td scope="row">${element.getServiceName()}</td>
						<td scope="row">${element.getUnit()}</td>
						<td scope="row">${element.getPrice()}</td>
						<td scope="row"><a
							href="editService?id=<c:out value='${element.getServiceID()}' />"><img
								src='<c:url value="/resources/images/edit.png" />'></a> <a
							href="deleteService?id=<c:out value='${element.getServiceID()}' /> "
							style=""><img
								src='<c:url value="/resources/images/quit.png" />'></a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
			...
			<ul class="pagination justify-content-end">
				<c:if test="${currentPage != 1}">
					<li class="page-item"><a class="page-link"
						href="danhsachdichvu?currentPage=${currentPage-1}">Previous</a></li>
				</c:if>
				...
				<c:forEach begin="1" end="${noOfPages}" var="i">
					<c:choose>
						...
						<c:otherwise>
							<li class="page-item"><a class="page-link"
								href="danhsachdichvu?currentPage=${i}">${i}</a></li>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				...
				<c:if test="${currentPage lt noOfPages}">
					<li class="page-item"><a class="page-link"
						href="danhsachdichvu?currentPage=${currentPage+1}">Next</a></li>
				</c:if>
			</ul>
		</nav>
	</div>
